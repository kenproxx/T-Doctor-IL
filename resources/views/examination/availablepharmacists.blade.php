    ...
    <div id="title" class="d-flex justify-content-center">
        <div class="list-title d-flex">
            <div class="list--doctor p-0">
                <a class="back" href="{{route('examination.findmymedicine')}}"><p><i class="bi bi-arrow-left"></i> {{ __('home.24/7 Available Pharmacists') }}</p></a>
            </div>
        </div>
    </div>
    <div class="list-doctor row">
        @foreach($availablePhamrmacists as $availablePhamrmacist)
            <div class=" col-md-3">
                <div class="card">
                <i class="bi bi-heart"></i>
                @php
                    $arrayGallery=[];
                    $gallery = $availablePhamrmacist->gallery;
                    if ($gallery){
                        $arrayGallery = explode(',', $gallery);
                    }
                    $text = '';
                    switch ($availablePhamrmacist->time_work){
                ...
                <div class="card-body">
                    <a href="#"><h5
                            class="card-title"> {{ $availablePhamrmacist->name }}</h5></a>
                    <p class="card-text_1">{{ __('home.Location') }}: <b>{{ $availablePhamrmacist->address_detail }}</b></p>
