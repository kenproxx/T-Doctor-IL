@php
    $showAll = \Illuminate\Support\Facades\DB::table('clinics')->get();
        ...
        @foreach($showAll as $show)
            <div class="col-md-4 mb-md-3">
                <div class="clinic-item">
                    ...
                    @else
                        <a href="#" data-toggle="modal" data-target="#staticBackdrop">Đăng nhập để xem chi tiết</a>
                    @endif
                    <div class="time d-flex">
                        <p>{{$show->open_date}} - {{$show->close_date}} </p>
                    </div>
                    <div class="location">
                        <i class="fa-solid fa-location-dot"></i> {{$show->address}} - <span>3 Km</span>
                    </div>
                    <div class="service">
                        Service: Implant, Brightening, Crown(null)
                    </div>
