<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('new_events', function (Blueprint $table) {
            $table->id();
            //title, title_en, title_laos, short_description, short_description_en, short_description_laos, description, description_en, description_laos, status, thumbnail, user_id
            $table->string('title')->nullable();
            $table->string('title_en')->nullable();
            $table->string('title_laos')->nullable();
            $table->string('short_description')->nullable();
            $table->string('short_description_en')->nullable();
            $table->string('short_description_laos')->nullable();
            $table->longText('description')->nullable();
            $table->longText('description_en')->nullable();
            $table->longText('description_laos')->nullable();
            $table->string('status')->nullable();
            $table->mediumText('thumbnail')->nullable();
            $table->unsignedBigInteger('user_id')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('new_events');
    }
};
